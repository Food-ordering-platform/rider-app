...
import { RiderEarningsResponse, RiderOrder, PayoutRequest, Bank } from "../../types/rider.types";
...
export const riderService = {
  ...
  getAvailableOrders: async (): Promise<RiderOrder[]> => {
    const response = await api.get<{ success: boolean; data: RiderOrder[] }>("/rider/orders/available");
    return response.data.data;
  },

  getActiveOrder: async (): Promise<RiderOrder | null> => {
    const response = await api.get<{ success: boolean; data: RiderOrder | null }>("/rider/orders/active");
    return response.data.data;
  },
   ...
   */
  rejectOrder: async (orderId: string, reason?: string): Promise<RiderOrder> => {
    const response = await api.patch<{ success: boolean; data: RiderOrder }>(`/rider/orders/${orderId}/reject`, { reason });
    return response.data.data;
  },
  ...
  confirmPickup: async (orderId: string): Promise<RiderOrder> => {
    const response = await api.patch<{ success: boolean; data: RiderOrder }>(`/rider/orders/${orderId}/pickup`);
    return response.data.data;
  },
   ...
   */
  confirmDelivery: async (orderId: string, code: string): Promise<RiderOrder> => {
    const response = await api.patch<{ success: boolean; data: RiderOrder }>(`/rider/orders/${orderId}/deliver`, { code });
    return response.data.data;
  },
  ...
  getEarnings: async (): Promise<RiderEarningsResponse> => {
    const response = await api.get<{ success: boolean; data: RiderEarningsResponse }>("/rider/earnings");
    return response.data.data;
  },
